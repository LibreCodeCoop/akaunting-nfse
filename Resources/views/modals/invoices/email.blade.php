            ...
            <x-tabs.tab id="general">
                <x-form.section>
                    <x-slot name="body">
                        ...
                        <x-form.group.text
                            name="subject"
                            label="{{ trans('settings.email.templates.subject') }}"
                            value="{{ $notification->getSubject() }}"
                            form-group-class="sm:col-span-6"
                        />

                        <x-form.group.editor
                            name="body"
                            label="{{ trans('settings.email.templates.body') }}"
                        ...
                        <x-form.group.checkbox
                            name="user_email"
                            :options="['1' => trans('general.email_send_me', ['email' => user()->email])]"
                            checkbox-class="col-span-6"
                            form-group-class="sm:col-span-6 -mb-8"
                        />
                ...
                <x-table>
                    <x-table.thead>
                        <x-table.tr class="flex items-center px-1">
                            ...
                            <x-table.th class="w-1/6"></x-table.th>
                            <x-table.th class="w-4/6">{{ trans('general.name') }}</x-table.th>
                            <x-table.th class="w-1/6">{{ trans('general.size') }}</x-table.th>
                        </x-table.tr>
                    </x-table.thead>
                    ...
                    <x-table.tbody>
                        ...
                        <x-table.tr id="method-pdf">
                            ...
                            <x-table.td class="w-1/6">{{ trans('general.na') }}</x-table.td>
                        </x-table.tr>

                        {{-- NFS-e DANFSE PDF --}}
                        <x-table.tr id="method-nfse-danfse">
                            <x-table.td class="w-1/12">
                                <input type="checkbox"
                                    id="attachment-nfse-danfse"
                                    name="nfse_attach_danfse"
                                    value="1"
                                    class="rounded-sm text-purple border-gray-300 cursor-pointer disabled:bg-gray-200 focus:outline-none focus:ring-transparent {{ $hasDanfse ? '' : 'opacity-50 cursor-not-allowed' }}"
                                    {{ $hasDanfse ? 'checked' : 'disabled' }}
                                    data-field="nfse_attachments"
                                    @input="e => form.attachments[e.target.name] = e.target.checked | 0">
                            </x-table.td>
                            <x-table.td class="w-1/6">
                                <div class="avatar-attachment">
                            ...
                            <x-table.td class="w-4/6">
                                {{ trans('nfse::general.invoices.emit_modal_email_attach_danfse') }}
                                @unless($hasDanfse)
                                    <span class="text-xs text-gray-400 ml-1">({{ trans('nfse::general.invoices.artifact_not_found') }})</span>
                                @endunless
                            </x-table.td>
                            <x-table.td class="w-1/6">{{ trans('general.na') }}</x-table.td>
                        </x-table.tr>
                        ...
                        {{-- NFS-e XML --}}
                        <x-table.tr id="method-nfse-xml">
                            <x-table.td class="w-1/12">
                                <input type="checkbox"
                                    id="attachment-nfse-xml"
                                    name="nfse_attach_xml"
                                    value="1"
                                    class="rounded-sm text-purple border-gray-300 cursor-pointer disabled:bg-gray-200 focus:outline-none focus:ring-transparent {{ $hasXml ? '' : 'opacity-50 cursor-not-allowed' }}"
                                    {{ $hasXml ? 'checked' : 'disabled' }}
                                    data-field="nfse_attachments"
                                    @input="e => form.attachments[e.target.name] = e.target.checked | 0">
                            </x-table.td>
                            <x-table.td class="w-1/6">
                            ...
                            <x-table.td class="w-4/6">
                                {{ trans('nfse::general.invoices.emit_modal_email_attach_xml') }}
                                @unless($hasXml)
                                    <span class="text-xs text-gray-400 ml-1">({{ trans('nfse::general.invoices.artifact_not_found') }})</span>
                                @endunless
                            </x-table.td>
                        ...
                        @if ($invoice->attachment)
                            @foreach($invoice->attachment as $attachment)
                                <x-table.tr id="method-{{ $attachment->id }}">
                                    ...
                                    <x-table.td class="w-4/6">{{ $attachment->basename }}</x-table.td>
                                    <x-table.td class="w-1/6">{{ ! is_int($attachment->size) ? '0 B' : $attachment->readableSize() }}</x-table.td>
                                </x-table.tr>
                            @endforeach
                        @endif
                    </x-table.tbody>
